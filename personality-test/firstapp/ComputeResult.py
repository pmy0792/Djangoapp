...
def calc_result(user):
    ...
    vote=Voting.objects.filter(user_ip__user_ip=user).values('question','choice')
    #choice=list(vote.values('choice'))[-12:] # the latest 12 choices of the user
    q_and_c=vote.order_by('-id')[:12]
    for qc in q_and_c:
    ...
    mbti_type=""
    mbti_type+=("E" if points['EI']>=2 else "I")
    mbti_type+=("N" if points['NS']>=2 else "S")
    mbti_type+=("T" if points['TF']>=2 else "F")
